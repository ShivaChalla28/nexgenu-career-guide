...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('http://localhost:8000/api/feedback/pending').then(res => res.json()),
      fetch('http://localhost:8000/api/feedback/approved').then(res => res.json())
    ])
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:8000/api/feedback/${id}/approve`, { method: 'PUT' });
      if (res.ok) {
  ...
  const handleReject = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:8000/api/feedback/${id}`, { method: 'DELETE' });
      if (res.ok) {
        setPendingFeedback(prev => prev.filter(f => f.id !== id));
